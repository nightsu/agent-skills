...
| 阶段 C-up (`api-mapping.md`,MVP 手填) | A + B 产物存在 | 阻塞,提示缺哪份 |
| 阶段 C-low (`figma-ui-api-mapper`) | A + B + `api-mapping.md` 存在 | 阻塞 |
...
> ⚠ **MVP 第 1 版的退让:** A/B/C-up 是用户手填的,套件无法保证语义质量。
