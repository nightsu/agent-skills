...
Phase E review gate 通过并选择 handoff 后,才进入 apply stage 写业务代码
```
...
- 收紧 `figma-workflow` / `figma-emit-spec` 的 apply 边界文案和 handoff 行为。
- 接入外部 handoff:`superpowers:brainstorming` 和 OpenSpec。
- 增加 v2 fixture,验证 "spec generation" 与 "code apply" 是两个阶段。
...
- `implementation-spec.md` 可以作为 apply stage 的主输入。
...
v2 因此不优先追求更多视觉能力,而是先补齐 A/B 前置语义产物、外部 handoff 与 apply 边界。
...
## Golden Rule: Apply Boundary

所有 v2 skill 必须遵守:

1. Phase A 到 E 只允许创建或更新 `docs/design/<feature>/` 下的设计实现产物。
2. 不修改业务代码目录,例如 `src/`, `app/`, `components/`, `pages/`。
3. `figma-emit-spec` 的输出 `implementation-spec.md` 是 apply stage 的单一主输入。
4. 只有用户在 Phase E review gate 选择 Proceed,并在 handoff 菜单选择 builtin / superpowers / manual 后,才进入 apply stage。
5. apply stage 如果由 agent 执行,必须显式声明它正在消费 `implementation-spec.md`,而不是 raw Figma JSON 或上游五份产物。
...
- 选择 `[3] Manual` 不产出额外文件,只提示用户自行把 `implementation-spec.md` 交给外部 apply 工具。
- 选择 `[4] Pause` 不产出额外文件。

外部 handoff 补充:

- `superpowers:brainstorming` 只作为 Phase A 的可选辅助入口,用于把模糊需求整理成 `clarified-requirement.md` 的输入事实。
- OpenSpec 只作为 handoff 目标被记录和提示;v2 不自动生成 OpenSpec proposal。
...
- `implementation-spec.md` 模板保持向后兼容,只补充 apply boundary 文案。
...
- 第 2 版: `figma-clarify-requirement` + `figma-ui-understand` + 外部 handoff / apply boundary。
